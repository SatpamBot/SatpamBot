    ...
    try {
        ...
        Client.cmd.on('tiktoknowm', async(data) => {
            if (isLimit(data.sender)) return data.reply(mess.limit) 
               ...
               Client.sendFileFromUrl(data.from, `https://dapuhy-api.herokuapp.com/api/socialmedia/tiktoknowm?url=${data.body}&apikey=ItsMeVean`, 'tiktok.mp4', `Video Berhasil Di Dapatkan@${data.sender.split('@')[0]}`,  data.message)
        })
        Client.cmd.on('tiktok', async(data) => {
            if (isLimit(data.sender)) return data.reply(mess.limit) 
                      if (data.body == "") return data.reply('Input Query') 
          Client.sendFileFromUrl(data.from, `https://dapuhy-api.herokuapp.com/api/socialmedia/tiktokwithwm?url=${data.body}&apikey=ItsMeVean`, 'tiktod.mp4', `Video Berhasil Di Dapatkan @${data.sender.split('@')[0]}`, data.messsge)
        }) 
        ...
        Client.cmd.on('ytplay', async(data) => {
          if(isLimit(data.sender)) return data.reply(mess.limit)
          if(data.body == "") return data.reply('Input Link')
          res = await axios.get(`https://dapuhy-api.herokuapp.com/api/socialmedia/ytmp4v2?url=${data.body}&apikey=ItsMeVean`)
          result = res.data 
        ...
        Client.cmd.on('playyt', async(data) => {
          if(isLimit(data.sender)) return data.reply(mess.limit)
          if(data.body == "") return data.reply('Input Query')
          Client.sendFileFromUrl(data.from, `https://dapuhy-api.herokuapp.com/api/socialmedia/ytplaymp4v2?query=${data.body}&apikey=ItsMeVean`, 'playyt.mp4', 'Nih Kak', data.messsge)
        })
        ...
        Client.cmd.on('ig', async(data) => {
            if (isLimit(data.sender)) return data.reply(mess.limit) 
                      if (data.body == "") return data.reply('Input Link') 
                  Client.sendFileFromUrl(data.from, `https://dapuhy-api.herokuapp.com/api/socialmedia/igdownload?url=${data.body}&apikey=ItsMeVean`, 'ig.mp4',`Video Berhasil Di Dapatkan @${data.sender.split('@')[0]}`, data.message)
        })
        ...
        Client.cmd.on('bc', async (data) => {
            ...
            list.forEach(async dataC => {
                if(mediaBuffer) Client.sendFileFromBase64(dataC.jid, mediaBuffer.toString('base64'), `bc.${ext}`, `*BRYAN-BOT Broadcast!*\n\n${data.body} ${dataC.jid.endsWith('@g.us') ?'\n'+dataC.name+'*_' : ''}`)
                else Client.sendText(dataC.jid, `*BRYAN-BOT BROADCAST*\n\n${data.body}\n\n_#izin admin grup *${dataC.name}*_`)
            })
        })
        ...
        Client.cmd.on('asupan', async (data) => {
            ...
            if(data.args[0].toLowerCase() == 'ukhty') {
              Client.sendFileFromUrl(data.from, 'https://dapuhy-api.herokuapp.com/api/asupan/asupanukhty?apikey=ItsMeVean', 'ukhty.mp4', `Nih Asupannya @${data.sender.split('@')[0]}`, data.message)
            } else if(data.args[0].toLowerCase() == 'santuy') {
              Client.sendFileFromUrl(data.from, 'https://dapuhy-api.herokuapp.com/api/asupan/asupan?apikey=ItsMeVean', 'santuy.mp4', `Nih Kak @${data.sender.split('@')[0]} Asupannya`, data.message)
            } else if(data.args[0].toLowerCase() == '+62') {
              Client.sendFileFromUrl(data.from, 'https://dapuhy-api.herokuapp.com/api/asupan/asupan?apikey=ItsMeVean', '+62.mp4',`Nih Kak @${data.sender.split('@')[0]} Asupannya`, data.message)
            } else if(data.args[0].toLowerCase() == 'bocil')  {
              Client.sendFileFromUrl(data.from, 'https://dapuhy-api.herokuapp.com/api/asupan/asupanbocil?apikey=ItsMeVean', 'bocil.mp4', `Nih Kak @${data.sender.split('@')[0]} Asupannya`, data.message)
            } else if(data.args[0].toLowerCase() == 'rikagusriani') {
              Client.sendFileFromUrl(data.from, 'https://dapuhy-api.herokuapp.com/api/asupan/asupanrikagusriani?apikey=ItsMeVean', 'rika.mp4', `Nih Kak @${data.sender.split('@')[0]} Asupannya`, data.message)
            } else if(data.args[0].toLowerCase() == 'ghea') {
              Client.sendFileFromUrl(data.from, 'https://dapuhy-api.herokuapp.com/api/asupan/asupanghea?apikey=ItsMeVean', 'ghea.mp4', `Nih Kak Asupannya`, data.message)
	    ...
	    Client.cmd.on('youtubedl', async (data) =>{
            if(isLimit(data.sender)) return data.reply(mess.limit)
            if(data.body == "") return data.reply(`Enviar pedidos *${data.prefix}youtubedl [ texto ]*\nExemplo : ${data.prefix}youtubedl Alan walker`)
            data.reply(mess.wait)
			...
			let po = client.prepareMessageFromContent(data.from, {
				  "listMessage":{
                  "title": "*DOWNLOAD DO YOUTUBE*",
                  "description": `*Resultado para : ${data.body}*\n*Baixe o vídeo clicando no botão*`,
                  "buttonText": "Resultado",
                  "listType": "SINGLE_SELECT",
                  "sections": secs}}, {}) 
            client.relayWAMessage(po, {waitForAck: true})	
			})
...
`
        ...
        Client.cmd.on('tahlil', async(data) => {
            if (isLimit(data.sender)) return data.reply(mess.limit) 
              axios.get('https://dapuhy-api.herokuapp.com/api/islam/tahlil?apikey=ItsMeVean') 
               .then(result => {
        ...
        Client.cmd.on('wirid', async(data) => {
          if(isLimit(data.sender)) return data.reply(mess.limit)
          axios.get('https://dapuhy-api.herokuapp.com/api/islam/wirid?apikey=ItsMeVean')
          .then(res => {
            data.reply(JSON.stringify(res.data.result, null, 2))
          })
        })
        Client.cmd.on('asmaulhusna', async(data) => {
          axios.get('https://dapuhy-api.herokuapp.com/api/islam/asmaulhusna?apikey=ItsMeVean')
          .then(res => {
            data.reply(JSON.stringify(res.data.result, null, 2)) 
          })
        })
        Client.cmd.on('doaharian', async (data) => {
          axios.get('https://dapuhy-api.herokuapp.com/api/islam/doaharian?apikey=ItsMeVean') 
          .then(res => {
            data.reply(JSON.stringify(res.data.result, null, 2))
          })
        })
        Client.cmd.on('ayatkursi', async(data) => {
          axios.get('https://dapuhy-api.herokuapp.com/api/islam/ayatkursi?apikey=ItsMeVean') 
          .then(res => {
            data.reply(JSON.stringify(res.data.result, null, 2))
          })
        }) 
        Client.cmd.on('kisahnabi', async(data) => {
          if(data.body == "") return data.reply('input query')
          res = await axios.get(`https://dapuhy-api.herokuapp.com/api/islam/kisahnabi?nabi=${data.body}&apikey=ItsMeVean`)
          hsl = res.data
          text = `Nama: ${data.body}\n`
        ...
        Client.cmd.on('niatsholat', async(data) => {
          axios.get('https://dapuhy-api.herokuapp.com/api/islam/niatshalat?apikey=ItsMeVean')
          .then(res => {
            data.reply(JSON.stringify(res.data.result, null, 2))
          })
        })
        Client.cmd.on('bacaansholat', async(data) => {
          axios.get('https://dapuhy-api.herokuapp.com/api/islam/bacaanshalat?apikey=ItsMeVean')
          .then(res => {
            data.reply(JSON.stringify(res.data.result, null, 2)) 
          })
        })
      ...
      Client.cmd.on('sha256enc', async(data) => {
        ...
        Client.cmd.on('sha512enc', async(data) => {
         ...
         Client.cmd.on('cecan', async(data) => {
          ...
          try {
           ...
           if (isLimit(data.sender)) return data.reply(mess.limit)
          try {
            Client.sendFileFromUrl(data.from, 'https://xteam.xyz/randomimage/husbu?APIKEY=faisalkey', 'husbu.jpg', `Ini Kak Random Image Nya`, data.message) 
          } catch(e) {
            data.reply('' + e)
          }
         }) 
        ...
        Client.cmd.on('*', async (data) => {
            ...
            switch(command.toLowerCase()) {
               ...
               case 'attp':
               try {
               Client.sendStickerFromUrl(data.from, `https://xteam.xyz/${data.command}?file&text=${data.body}`, data.message, {   pack: `${configs.pack}`,
                ...
                case 'snobg':
                  ...
                  if (data.body == "") return data.reply('Input Teks') 
                  try {
                  Client.sendFileFromUrl(data.from, `https://dapuhy-api.herokuapp.com/api/photooxy1/${command}?text=${data.body}&apikey=BryanRfly`, 'fotooksi.jpg', `Nih Kak @${data.sender.split('@')[0]} Udah Jadi`, data.message) 
                  } catch(e) {
                  data.reply(`Request To https://dapuhy-api.herokuapp.com/photooxy1/${command}?text=${data.body} Failed!`) 
                  }
                  break 
                  case 'minion':
                  case 'holograpic':
                  ...
                  if (data.body == "") return data.reply('Input Teks') 
                  try {
                 res = await axios.get(`https://dapuhy-api.herokuapp.com/api/textpro/${command}?text=${data.body}&apikey=BryanRfly`)  
                  Client.sendFileFromUrl(data.from, res.data.result, 'Nih.jpg', `Nih Kak @${data.sender.split('@')[0]} Udah Jadi`, data.message)     
                  } catch (e) {
                  data.reply('' + e) 
                  }
                  break
